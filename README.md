...
```bash
npm run generate prompts/reminders.md
```
...
# Run a specific test
npm run verify tests/reminders.test.js

# Or use wdio directly
npm test -- --spec tests/reminders.test.js
```
...
│   ├── reminders.md                  # Example: Reminders app test
...
│   └── reminders.test.js             # Generated test (example)
...
```bash
npm run generate prompts/reminders.md
npm run generate prompts/reminders.md --no-run  # Generate without running
```
...
```javascript
/**
 * Generated from: prompts/reminders.md
